...
int main(int argc, char* argv[])
{
    ...
    const char* tempFileName = "C:/Users/1642645/source/repos/bmp_tasks/txt_file.txt";
    ...
    // создание семафора
    semaph = CreateSemaphore(NULL, process_number, process_number, NULL);
    if (!semaph) printf("Error CreateSemaphore");

    if (argc == 1)
    {
        FILE* tempFileCreate = fopen(tempFileName, "w");
        fclose(tempFileCreate);
        ...
        if (tempFile.is_open())
        {
            ...
            {
                sscanf(line.c_str(), "%d %d", &tempLower, &tempHigher);
    ...
    else
    {
        ...
        char processResult[defSize];
        sprintf(processResult, "%d %d", lowerCounter, higherCounter);

        // запись результатов из всех процессов в один файл
        DWORD result = WaitForSingleObject(semaph, 0L);
        if (result == WAIT_OBJECT_0)
        {
            ofstream log(tempFileName, ios_base::app | ios_base::out);
            log << processResult << "\n";
            if (!ReleaseSemaphore(semaph, 1, NULL)) printf("Error ReleaseSemaphore");
        }
